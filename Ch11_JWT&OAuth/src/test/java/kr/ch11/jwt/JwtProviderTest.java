...
public class JwtProviderTest {
    ...
    public void generateTokenTest() {
    	...
    	log.info("generateTokenTest token : " + token);

    	// then - 테스트의 결과를 검증하는 단계로 AssertJ 검증문 이용
    	Claims claims = jwtProvider.getClaims(token);
    	String uid = (String) claims.get("uid");
    	log.info("generateTokenTest uid : " + uid);
    ...
    public void validTokenTest1() {
		...
		log.info("validTokenTest token : " + token);

		// when
		boolean result = jwtProvider.validateToken(token);
		log.info("validTokenTest result : " + result);
    ...
    public void validTokenTest2() {
		...
		log.info("validTokenTest2 token : " + token);

		// when
		boolean result = jwtProvider.validateToken(token);
		log.info("validTokenTest2 result : " + result);
